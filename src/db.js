...
try { db.exec("ALTER TABLE ai_profiles ADD COLUMN sex_fantasmes TEXT NOT NULL DEFAULT ''"); } catch (_) {}
try { db.exec("ALTER TABLE ai_profiles ADD COLUMN sex_practiced TEXT NOT NULL DEFAULT ''"); } catch (_) {}
try { db.exec("ALTER TABLE ai_profiles ADD COLUMN sex_details TEXT NOT NULL DEFAULT ''"); } catch (_) {}
...
function createAiProfile({ name, description, fantasmes, age, sexe, physical_desc, relation_type, sex_fantasmes, sex_practiced, sex_details }) {
  const r = db.prepare(`INSERT INTO ai_profiles (name, description, fantasmes, age, sexe, physical_desc, relation_type, sex_fantasmes, sex_practiced, sex_details) VALUES (?,?,?,?,?,?,?,?,?,?)`)
    .run(name || '', description || '', JSON.stringify(fantasmes || []), age || '', sexe || '', physical_desc || '', relation_type || '', sex_fantasmes || '', sex_practiced || '', sex_details || '');
  return r.lastInsertRowid;
}
function updateAiProfile(id, { name, description, fantasmes, age, sexe, physical_desc, relation_type, sex_fantasmes, sex_practiced, sex_details }) {
  db.prepare(`UPDATE ai_profiles SET name=?, description=?, fantasmes=?, age=?, sexe=?, physical_desc=?, relation_type=?, sex_fantasmes=?, sex_practiced=?, sex_details=?, updated_at=unixepoch() WHERE id=?`)
    .run(name || '', description || '', JSON.stringify(fantasmes || []), age || '', sexe || '', physical_desc || '', relation_type || '', sex_fantasmes || '', sex_practiced || '', sex_details || '', id);
}
